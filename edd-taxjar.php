...
class EDD_TaxJar {
	...
	public function update_order( $payment_id, $payment ) {
		...
		if( 'complete' !== $payment->status && 'publish' !== $payment->status ) {
	...
	private function build_order( EDD_Payment $payment ) {
		...
		$order = array(
			...
			'to_country'       => $payment->user_info['address']['country'],
			'to_zip'           => $payment->user_info['address']['zip'],
			'to_state'         => $payment->user_info['address']['state'],
			'to_city'          => $payment->user_info['address']['city'],
			'to_street'        => trim( $payment->user_info['address']['line1'] . ' ' . $payment->user_info['address']['line1'] ),
